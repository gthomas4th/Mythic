...
public struct ROMIndex: Codable, Sendable {
    ...
    public mutating func scan(root: URL, system: String) throws {
        ...
        while let url = iterator.nextObject() as? URL {
            ...
            if ["nes", "sfc", "smc", "gb", "gbc", "gba", "n64", "z64", "iso", "chd", "cue", "m3u", "gcm", "rvz", "wia", "wbfs", "ciso"].contains(url.pathExtension.lowercased()) || url.lastPathComponent.uppercased() == "EBOOT.BIN" { candidates.append(url.resolvingSymlinksInPath()) }
        }
