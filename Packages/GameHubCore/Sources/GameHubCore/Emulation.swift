...
public enum EmulatorKind: String, Codable, CaseIterable, Sendable { case retroArch, duckStation, pcsx2, rpcs3 }
public enum EmulatorCommand {
    public static func arguments(kind: EmulatorKind, content: URL, core: URL? = nil) throws -> [String] {
...
public enum ROMError: LocalizedError {
    case invalid, coreMissing, missingPart
...
public struct ROMIndex: Codable, Sendable {
    ...
    public mutating func scan(root: URL, system: String) throws {
        ...
        while let url = iterator.nextObject() as? URL {
            ...
            if ["nes", "sfc", "smc", "gb", "gbc", "gba", "n64", "z64", "iso", "chd", "cue", "m3u"].contains(url.pathExtension.lowercased()) || url.lastPathComponent.uppercased() == "EBOOT.BIN" { candidates.append(url.resolvingSymlinksInPath()) }
        }
        var suppressed: Set<String> = []
        for descriptor in candidates where ["cue", "m3u"].contains(descriptor.pathExtension.lowercased()) {
        ...
        for url in candidates.sorted(by: { $0.path < $1.path }) where !suppressed.contains(url.resolvingSymlinksInPath().path) {
            ...
            if let old = cache[relative], old.signatures == signatures { fingerprint = old.fingerprint } else {
                ...
                let payloads = parts.filter { !["cue", "m3u"].contains($0.pathExtension.lowercased()) }
    ...
    public static func parts(of file: URL, root: URL, depth: Int = 0) throws -> [URL] {
        ...
        guard ["cue", "m3u"].contains(ext) else { return [file] }
