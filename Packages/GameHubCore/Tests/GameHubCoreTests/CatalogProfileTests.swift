...
final class CatalogProfileTests: XCTestCase {
    ...
    func testFirstSeenSurvivesRescanPreferenceChangesAndReopen() throws {
        ...
        do {
            ...
            try store.setPreference(.init(favorite: true, preferredTargetID: "remote"), for: record.id.description)
            let renamed = GameRecord(id: record.id, title: "Updated title", launchTargets: [], artwork: URL(string: "https://example.invalid/cover.jpg"))
            try store.upsert([renamed], observedAt: later)
        ...
        XCTAssertEqual(try store.records().first?.title, "Updated title")
        XCTAssertEqual(try store.records().first?.artwork?.absoluteString, "https://example.invalid/cover.jpg")
